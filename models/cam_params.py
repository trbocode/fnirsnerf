...
class CamParams(nn.Module):
    ...
    def from_config(num_imgs: int, H0: float = 1000, W0: float = 1000,
        so3_repr: str = 'axis-angle', intr_repr: str = 'square', initial_fov: float = 53., our_phi=None, our_t=None):
        ...
        if (our_phi is not None) and (our_t is not None):
            phi = torch.from_numpy(our_phi).float()
            t = torch.from_numpy(our_t).float()
...
def get_camera2world(rot, trans, representation='quaternion'):
    ...
    homo_m = torch.cat((tmp.float(), extend), dim=-2)   # [...,4,4]
