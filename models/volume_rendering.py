...
def volume_render(
    ...
    **kwargs):
    ...
    def render_rayschunk(rays_o, rays_d):
        #print(rays_o.shape,"Raychunking")
        if use_viewdirs:
        ...
        if N_importance > 0:
            ...
            all_raw = coarse_raw
            all_z_vals = z_vals
        # We need to filter using pts and set, for example, sigma to 90 and color to 1 0 0
        # Xpts= pts[:,:,:,0]
        # Ypts= pts[:,:,:,1]
        # Zpts= pts[:,:,:,2]
        # # -0.46703328016070295 0.38612678492342506
        # # -0.5934465183983699 0.2597135466857581
        # # -0.3260959982872009 0.5270640667969271

        # # #Xpts= torch.ge(Xpts,-0.1) & torch.le(Xpts,0.4) #Left to right for real
        # # #Ypts= torch.ge(Ypts,-0.5) & torch.le(Ypts,0.0) # Top to buttom
        # # #Zpts= torch.ge(Zpts,0.2) & torch.le(Zpts,0.7) # Actual height, keep for Doll model!
        # Xpts= torch.ge(Xpts,-0.46703328016070295) & torch.le(Xpts,0.38612678492342506) #Left to right for real
        # Ypts= torch.ge(Ypts,-0.5934465183983699) & torch.le(Ypts,0.2597135466857581) # Top to buttom
        # Zpts= torch.ge(Zpts,-0.3260959982872009) & torch.le(Zpts,0.5270640667969271) # Actual height, lower numbers are higher. keep for Doll model!
        # bads=Xpts & Ypts & Zpts
        # all_raw['sigma']=all_raw['sigma']*bads # Filter the points, delete if not using this 
        # ---------------
...
def batchify_query_network(
    ...
    *_, N_rays, N_samples, _ = pts.shape
    ...
    for i in range(0, pts.shape[DIM_TO_BATCHIFY], netchunk):
        # print()
        # print("shape of slice_chunk ",slice_chunk(pts,i,netchunk).shape)
